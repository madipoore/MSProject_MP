...
def SCS_L2est(dates, re, market, freq, anomalies, parameters):
    ...
    if isinstance(oos_date, str):
        ...
        raise TypeError(f"oos_test_date has unexpected type: {type(oos_date)} — expected str or Timestamp/datetime")

    # Quick confirmation (you can remove this print later)
    ...
    # Ensure index is datetime and sorted (critical!)
    if not pd.api.types.is_datetime64_any_dtype(re.index):
        print("Converting index to datetime...")
        re.index = pd.to_datetime(re.index, errors='coerce')  # coerce bad values to NaT
        market.index = pd.to_datetime(market.index, errors='coerce')

    # Drop any NaT rows if parsing failed (rare but safe)
    re = re[re.index.notna()]
    market = market[market.index.notna()]

    # Sort to make monotonic (required for reliable partial slicing)
    re = re.sort_index()
    market = market.sort_index()

    # Debug prints to confirm (remove later if you want)
    # print("re index dtype after fix:", re.index.dtype)
    # print("Is monotonic increasing?", re.index.is_monotonic_increasing)
    # print("First date:", re.index.min())
    # print("Last date:", re.index.max())
    # print("tT0:", tT0)
    # print("Number of obs before tT0:", (re.index <= tT0).sum())
    ...
    # Effective degrees of freedom (dynamic for any n)
    n = parameters['n']  # from earlier in the function (T, n = r_train.shape)
    df = np.sum(d.reshape(n,1) / (d.reshape(n,1) + np.array(l).reshape(1, parameters['gridsize'])), axis=0)
